...
export class OrderBusiness {
    ...
    public createOrder = async (input: ICreateOrderInputDTO): Promise<ICreateOrderOutputDTO> => {
        ...
        if (pizzasInput.length === 0) {
            throw new ParamsError("Pedido vazio. Selecione no mínimo uma pizza")
        }

        const pizzas = pizzasInput.map((pizza) => {
            if (pizza.quantity <= 0) {
                throw new ParamsError("Quantidade inválida. Pedido mínimo é 1")
            }
        ...
        const total = pizzas.reduce((a, b) => (a + (b.price * b.quantity)), 0)
